...
Current version employs Tensorflow version 2.18 that requires CUDA 12. Our installation recipe will
automatically install, among other packages, Tensorflow=2.18 and CUDA 12.1. You can always install the older version
that used Tensorflow 1.14 and cuda 10.2. If your drivers are not compatible with any of the configurations
and you cannot update them, you can try to compile tensorflow-gpu=2.18 using your library settings instead of
installing it using conda.

### Install from source option

The best option to keep you updated. Currently, it uses Tensorflow version 2.18 and CUDA 12.1. This option works
well in the new Nvidia GPUs (Ampere, Ada Lovelace) but is should also work for the older versions. See other installation options
if you want to install the original version of deepEMhancer that does NOT work in Nvidia Ampere GPUs.
...
```bash
conda create -n deepEMhancer_env python=3.12
```
...
3. Install deepEMhancer

```bash
conda install deepEMhancer -c rsanchez1369 -c anaconda -c conda-forge
```

4. Download our deep learning models

```bash
deepemhancer --download
```

5. Ready! Do not forget to activate the environment for future usages. For a complete help use:
...
1. Clone the tag "0.14" of the repository and cd inside
...
3. Install deepEMhancer (using Tensorflow 2.18)
...
- With GPU support
  - Install CUDA 12.1 and cudnn 9.1. Make sure that they are in the LD_LIBRARY_PATH. If you are in a conda environment you probably want to add permanently the following `export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$CONDA_PREFIX/lib/`
...
5. Ready! Do not forget to activate the environment, if used (step 2), for future usages. For a complete help use:
...
In other cases is related with an incompatibility between CUDA and cudnn versions.
...
### 3. Multiple GPU batch size issue
...
**Error:** It is not possible to download the models.

**Explanation:** Sometimes our server may not be reachable due to network problems.

**Solution:** Download them from <https://zenodo.org/record/7432763>
